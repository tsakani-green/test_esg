...
// Invoice Table Component – simplified (no Hide/Show, no Export, no View links)
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {invoices.map((invoice, index) => {
              ...
              const estimatedCarbon =
                (sixMonthEnergy || 0) * EF_ELECTRICITY_T_PER_KWH;
...
export default function EnvironmentalCategory() {
  ...
  const avgCarbon =
    co2Emissions.length > 0
      ? co2Emissions.reduce((s, v) => s + (v || 0), 0) /
        co2Emissions.length
      : 0;
      ...
      case "invoices":
        return (
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-white to-gray-50 rounded-3xl border border-gray-200 shadow-xl p-6">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Upload, view, and manage PDF invoices with extracted energy
                    and cost data
                  </p>
                </div>
            ...
            {/* Invoice Table */}
            <div className="bg-gradient-to-br from-white to-gray-50 rounded-3xl border border-gray-200 shadow-xl p-6">
              <InvoiceTable invoices={invoiceSummaries} />
            </div>
          ...
          <div className="space-y-6">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Upload bulk PDF invoices to extract energy, cost, and carbon
                    data for analysis
                  </p>
                </div>
                {invoiceSummaries.length > 0 && (
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-medium text-emerald-700">
                      {invoiceSummaries.length} invoice
                      {invoiceSummaries.length > 1 ? "s" : ""} processed
                    </span>
                    <button
                      onClick={handleInvoiceKpiClick}
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="divide-y divide-gray-200 bg-white">
                        {invoiceSummaries.slice(0, 5).map((invoice, index) => {
                          const sixMonthEnergy =
                            getInvoiceSixMonthEnergy(invoice);
            ...
            <aside className="bg-white rounded-3xl shadow border border-slate-200 p-5 flex flex-col h-full">
              ...
              <p className="text-xs text-slate-500 mt-1 mb-3">
                Generated commentary based on your uploaded environmental metrics
                and invoice data.
              </p>
